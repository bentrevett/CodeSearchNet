...
run_path = os.path.join('pred_runs/', *run_name)
...
fields = {'code_tokens': ('code', CODE), 'func_name': ('label', LABEL)}
...
if args.model == 'transformer':
    ...
    code_predictor = models.EmbeddingPredictor(args.emb_dim,
                                               len(LABEL.vocab),
                                               args.pool_mode)
...
if args.model == 'transformer':
    ...
    code_predictor.apply(utils.initialize_transformer)
...
print(f'Code Predictor parameters: {utils.count_parameters(code_predictor):,}')

optimizer = optim.Adam([{'params': code_encoder.parameters()},
                        {'params': code_predictor.parameters()}],
                        lr = args.lr)

if args.loss == 'softmax':
    criterion = utils.SoftmaxLossPred(device)
else:
    raise ValueError
...
def train(code_encoder, code_predictor, iterator, optimizer, criterion):
    ...
    for batch in tqdm(iterator, desc='Training...'):
        ...
        torch.nn.utils.clip_grad_norm_(code_predictor.parameters(), args.grad_clip)
...
for epoch in range(args.n_epochs):
    ...
    if valid_mrr > best_valid_mrr:
        ...
        if args.save_model:
            ...
            torch.save(code_predictor.state_dict(), os.path.join(run_path, 'code_predictor.pt'))
    else:
        patience_counter += 1
